...
def createDataset(outputPath, imagePathList, labelList, lexiconList=None, checkValid=True,gt_path=None):
    """
    ARGS:
    ...
    for i in range(nSamples):
        imagePath = gt_path+imagePathList[i]
        label = labelList[i]
...
def get_mydataset(gt_path,img_path):
...
if __name__ == '__main__':
    ...
    imgList, labelList = get_mydataset(gt_path,img_path)
    print("The length of the list is ", len(imgList))

    '''Input the address you want to generate the lmdb file.'''
    createDataset(img_path, imgList, labelList, gt_path=img_path)
