    ...
    for taski in range(len(train_datasets)):
        ...
        for iteration in tqdm(
            ...
            if iteration % opt.val_interval == 0 or iteration == 1:
                # for validation log
                val(model, criterion, valid_loader, converter, opt,optimizer,best_score,start_time,iteration,train_loss_avg,taski)
                train_loss_avg.reset()
                semi_loss_avg.reset()


        """ Evaluation at the end of training """
        best_scores, log = test(AlignCollate_valid,valid_datas,model,criterion,converter,opt,best_scores,taski,log)
...
def val(model, criterion, valid_loader, converter, opt,optimizer,best_score,start_time,iteration,train_loss_avg,taski):
    with open(f"./saved_models/{opt.exp_name}/log_train.txt", "a") as log1:
        ...
        valid_log += f'{"Ned_score":17s}: {ned_score:0.2f}'
