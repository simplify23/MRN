...
def train(opt, log):
    ...
    for taski in range(len(train_datasets)):
        ...
        log.write("Trainable params num: {:.2f} M\n".format(sum(params_num)/1000000))
            ...
            if iteration % opt.val_interval == 0 or iteration == 1:
                ...
                with open(f"./saved_models/{opt.exp_name}/log_train.txt", "a") as log1:
                    ...
                    if current_score > best_score:
                        ...
                        torch.save(
                            model.state_dict(),
                            f"./saved_models/{opt.exp_name}/{opt.lan_list[taski]}_best_score.pth",
                        )
        ...
        best_scores, log = test_for_task(AlignCollate_valid,valid_datas,model,criterion,converter,opt,best_scores,taski,log)
...
def test_for_task(AlignCollate_valid,valid_datas,model,criterion,converter,opt,best_scores,taski,log):
    print("---Start evaluation on benchmark testset----")
    """ keep evaluation model and result logs """
    os.makedirs(f"./result/{opt.exp_name}", exist_ok=True)
    os.makedirs(f"./evaluation_log", exist_ok=True)
    saved_best_model = f"./saved_models/{opt.exp_name}/{opt.lan_list[taski]}_best_score.pth"
